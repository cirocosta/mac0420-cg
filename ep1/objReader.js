...
var to_int_minus_1 = function (val) {
...
function parse (text) {
  ...
  text.split('\n').forEach(function (line) {
    ...
    switch (match[1]) {
      case 'v':
        var elems = line.split(' ').slice(1).map(to_float);
        var x = elems[0];
        var y = elems[1];
        var z = elems[2];
      ...
      case 'vn':
        Array.prototype.push.apply(result.vertices_normals,
                                   line.split(' ').slice(1).map(to_float));
        break;
      ...
      case 'f':
        ...
        if (facesType === FACES_TYPES.FACE) {
          ...
          // that
          if (!~found_index) {
            Array.prototype.push.apply(result.vertices_normals, face_normal);
        ...
        faces.forEach(function (face) {
          ...
          // 0-index
          var elems = face.split('//');
          var verticeI, normalI;
  ...
  if (result.vertices_coords.length) {
    ...
    result.center_of_mass =
      result.center_of_mass.map(function (elem) {
...
function getNormal (a, b, c) {
  var v1 = b.map(function (elem, i) { return elem - a[i]; });
  var v2 = c.map(function (elem, i) { return elem - a[i]; });
  var normal = [];
